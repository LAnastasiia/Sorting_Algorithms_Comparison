...
int main() {
    for(int arr_len = 128; arr_len <= 524288; arr_len *= 2){
        int *arr = (int *) malloc(arr_len * sizeof(int));                  // allocate memory for array
        int se_copy[arr_len], in_copy[arr_len], sh_copy[arr_len];          // initialize copies of initial array

        generateArray(arr, arr_len);                                       // make content for array

        printf("%d\n", arr_len);

        for(int i = 0; i < 8; i++){

            copyArray(arr, se_copy, arr_len);                             // make copies of initial array
            copyArray(arr, in_copy, arr_len);
            copyArray(arr, sh_copy, arr_len);

            // OUTPUT RESULTS
            printf("           %f\n", selectionSort(se_copy, arr_len));    // SELECTION SORT
            printf("           %f\n", insertionSort(in_copy, arr_len));    // INSERTION SORT
            printf("           %f\n", shellSort(sh_copy, arr_len));        // SHELL     SORT
        }
        printf("\n\n");
    }
}
